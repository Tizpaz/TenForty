...
#                 print("I/O error")
        ...
        if(property_id == 4):
            ...
            while((time.time() - start_time) <= max_time):
                inp = inp_0.copy()
                inp[L1_index ] = np.random.randint(56844,80000)
...
#                 print("I/O error")
        ...
        if(property_id == 5):
            Depend_ind = 5#2020
            L27_ind = 23 # Earned Income Credit (EIC)
            ...
            while((time.time() - start_time) <= max_time):
                ...
                inp[L1_ind] = np.random.randint(10000,56844)
                if(index == Depend_ind):
...
#                 print("I/O error")
        ...
        if(property_id == 10):
            L1_ind  = 7 #Wages, salaries, tips (W-2's Box-1).
            L29_ind = 25 #American Opportunity Credit 2020
            writer.write("AGI,")
            writer.write("ETC,")
            writer.write("return,")
            writer.write("failed\n")
...
#                 print("I/O error")
        ...
        if(property_id == 15):
            ...
            while((time.time() - start_time) <= max_time):
                inp = inp_0.copy()
                inp[L1_ind] = np.random.randint(1860,160001)
                inp[A5a_ind] = 0.0
                inp[A5b_ind] = 0.0
                AGI1 = AGI_calc(inp,cap_gain_ind, tot_inc_ind, char_ind, adj_to_inc_ind)
                #AGI1 = (inp[7] + inp[8] + inp[9] + inp[10] + inp[11] + inp[12] + inp[13] + inp[14] + inp[15] + inp[16])
                inp[A1_ind] = np.random.randint(AGI1 * 0.075, 24800 + AGI1 * 0.075)
                inp[A18_ind] = 1
                FTR1 = test_cases(input_program_tree, inp, writer)
                #handle_debugging(True, None, feature_val, csv_columns, 0)
                MDE1 = inp[A1_ind]
...
#                 print("I/O error")
        ...
        if(property_id == 16):
            ...
            while((time.time() - start_time) <= max_time):
                ...
                AGI1 = AGI_calc(inp,cap_gain_ind, tot_inc_ind, char_ind, adj_to_inc_ind)
                #AGI1 = (inp[7] + inp[8] + inp[9] + inp[10] + inp[11] + inp[12] + inp[13] + inp[14] + inp[15] + inp[16])
                if 24800+AGI1*0.075 > AGI1:
                    continue
                inp[A1_ind] = np.random.randint(24800+AGI1*0.075, AGI1)
                FTR1 = test_cases(input_program_tree, inp, writer)
                #handle_debugging(True, None, feature_val, csv_columns, 0)
                MDE1 = inp[A1_ind]
                inp_1 = inp.copy()
                ...
                while(K < 90):
                    ...
                    if(FTR2 - FTR1 > 1.0):
                        ...
                        writer.write("True\n")
                        results.append(True)
                        #andle_debugging(True, True, feature_val, csv_columns, K)
                    K = K + 1
                counter += 1
#             try:
#                 f_cov_writer = csv.DictWriter(f_cov, fieldnames=csv_columns)
...
#     input('dddddd')
    ...
    filename =  "US_1040_testing_married_joint_schedule_A_3_2020.txt"
    ...
    with open(filename, 'w') as out:
        ...
        out.write("SpouseOccupat: 13Welder\n")
        out.close()
    
    ret = call(["../../bin/taxsolve_US_1040_2020", filename])
    
    tax_return = 0.0
    with open("US_1040_testing_married_joint_schedule_A_3_2020_out.txt", 'r') as read_f:
        try:
            for line in read_f:
                if "L34" in line:
...
if __name__ == '__main__':
    ...
    property_num = int(args.property)
    max_time = int(args.timeout)
    res, time_1_failed = property_generation(property_num, start_time, max_time)

    f = open("Results_debugging_property_" + str(property_num) + "_" + str(start_time).split(".")[0] + ".txt" , 'w')
    f.write("Results_debugging_property_" + str(property_num) + ": ")
